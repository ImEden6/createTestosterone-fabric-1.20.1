...
import net.mervyn.testosterone.sounds.ratRollingSound;
...
public class ratEntity extends Animal implements PlayerRideableJumping {
    ...
    private final Object rollingSound;

    public ratEntity(EntityType<? extends Animal> type, Level level) {
        super(type, level);

        if (level.isClientSide()) {
            this.rollingSound = clientOnly.createRollingSound(this);
        } else {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new TemptGoal(this, 1.2D, Ingredient.of(testosteroneModItems.CHEESE_ON_A_STICK.get(), testosteroneModBlocks.CHEESE_BLOCK.get()), false));
    ...
    public boolean isFood(ItemStack pStack) {
        return pStack.is(testosteroneModBlocks.CHEESE_BLOCK.get().asItem());
    }
    ...
    static class clientOnly {
        static Object createRollingSound(ratEntity rat) {
            return new ratRollingSound(rat);
        }

        static void playRollingSound(ratEntity rat, Object rollingSound) {
            if (rollingSound instanceof ratRollingSound sound) {
