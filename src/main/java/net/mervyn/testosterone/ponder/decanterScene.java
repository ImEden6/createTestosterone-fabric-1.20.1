...
public class decanterScene {
    public static void decanter_main(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        AABB bbRed1 = new AABB(Vec3.ZERO, Vec3.ZERO).inflate(0.5, 0, 0.5);
        AABB bbRed2 = new AABB(Vec3.ZERO, Vec3.ZERO).inflate(0.5, 1.5, 0.5);
        ...
        FluidStack cholesterol = new FluidStack(testosteroneFluids.CHOLESTEROL_FLUID.get()
                .getSource(), 16000);
        scene.world().setKineticSpeed(decanter1sele, 0);
        scene.world().setKineticSpeed(decanter2sele, 0);

        scene.world().showSection(decanter1sele, Direction.DOWN);
        scene.idle(15);
        ...
        AABB bb1 = new AABB(Vec3.ZERO, Vec3.ZERO).inflate(0.5, 0.5, 0);
        AABB bb2 = new AABB(Vec3.ZERO, Vec3.ZERO).inflate(0.5, 0.5, 1.5);
        Object fluids = new Object();

        scene.world().showSection(tank1, Direction.WEST);
        scene.world().modifyBlockEntity(tankPos1, FluidTankBlockEntity.class, be -> be.getTankInventory()
                .setFluid(cholesterol.copy()));
        scene.idle(5);
        scene.world().showSection(tank2, Direction.WEST);
        ...
        for (int i = 0; i < 50; i++) {
            final int drainAmount = (i < 36) ? 400
                    ...
                    : (i == 37 ? 200
                    : 100));
            drainedTotal += drainAmount;
            final long remainingCholesterol = Math.max(16000 - drainedTotal, 0);
            final long producedTestosterone = (i + 1L) * 400L;
            scene.world().modifyBlockEntity(tankPos1, FluidTankBlockEntity.class,
                    be -> be.getTankInventory().setFluid(new FluidStack(
                            testosteroneFluids.CHOLESTEROL_FLUID.get().getSource(), remainingCholesterol)));
            scene.world().modifyBlockEntity(tankPos2, FluidTankBlockEntity.class,
                    be -> be.getTankInventory().setFluid(new FluidStack(
                            testosteroneFluids.TESTOSTERONE_FLUID.get().getSource(), producedTestosterone)));
            scene.idle(1);
        }
        scene.idle(25);
        ...
        scene.world().showSection(decanter2sele, Direction.DOWN);
        scene.idle(10);
        scene.world().setKineticSpeed(decanter2sele, 256);
        scene.idle(5);

        scene.overlay().showText(65)
                .pointAt(util.vector().blockSurface(decanter1, Direction.SOUTH))
                .placeNearTarget()
                .attachKeyFrame()
                .text("Centrifuges can be stacked on top of each other.");
        scene.overlay().showOutline(PonderPalette.RED, decanter1, decanter2sele, 65);
        scene.idle(80);
    }
}
